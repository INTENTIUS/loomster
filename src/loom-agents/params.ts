 ...
 *
 * The six cross-stack values (artifact bucket + ECS security group from
 * shared-foundation, the OAuth2 token/discovery URLs from loom-cognito) are
 * genuine CloudFormation `Parameter`s — real Ref()-able declarables,
 * resolved at deploy time by `../components/loom-agents.component.ts`'s
 * `cfn-deploy` step, keyed by export name so their logical id in the
 * synthesized template exactly matches the key that wiring uses.
 *
 ...
 * production/production-ha) — the same `LOOM_PRIVATE_SUBNET_IDS` baseline
 * `../loom-db/params.ts`/`../loom-backend/params.ts` read.
 */
